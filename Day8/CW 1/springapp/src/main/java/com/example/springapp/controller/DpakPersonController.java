...
public class DpakPersonController {
    ...
    @PostMapping("/person")
    public ResponseEntity<DpakPerson> post(@RequestBody DpakPerson person) {
        if (ser.post(person)) {
    ...
    public ResponseEntity<List<Person>> getAll(@PathVariable String value) {
        List<DpakPerson> li = ser.start(value);
        if (li.size() > 0) {
    ...
    public ResponseEntity<List<DpakPerson>> getbyAge(@PathVariable String value) {
        List<DpakPerson> li = ser.end(value);
        if (li.size() > 0) {
            return new ResponseEntity<>(li, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
